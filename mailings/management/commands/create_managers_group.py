...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--name',
            default='Managers',
            help="Имя группы (по умолчанию 'Managers')"
        )
        parser.add_argument(
            '--assign',
            nargs='*',
    ...
    def handle(self, *args, **options):
        ...
        for model, add_view, add_change, add_delete in (
            ...
            if add_view:
                try:
                    perms_to_add.append(Permission.objects.get(content_type=ct, codename=f'view_{model._meta.model_name}'))
                except Permission.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"Permission view_{model._meta.model_name} не найден для {model}"))
            if add_change:
                try:
                    perms_to_add.append(Permission.objects.get(content_type=ct, codename=f'change_{model._meta.model_name}'))
                except Permission.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"Permission change_{model._meta.model_name} не найден для {model}"))
            if add_delete:
                try:
                    perms_to_add.append(Permission.objects.get(content_type=ct, codename=f'delete_{model._meta.model_name}'))
                except Permission.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"Permission delete_{model._meta.model_name} не найден для {model}"))

        # права на работу с пользователями (view + change для блокировки)
        try:
            ct_user = ContentType.objects.get_for_model(User)
            perms_to_add.append(Permission.objects.get(content_type=ct_user, codename='view_user'))
            perms_to_add.append(Permission.objects.get(content_type=ct_user, codename='change_user'))
